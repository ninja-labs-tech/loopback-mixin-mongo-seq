...
module.exports = (app, { modelName, dataSource }) => {
  ...
  CounterModel.incrementSeq = async ({ name, step = 1 }) => {
    ...
    // if document was upserted then reincrement the sequence
    if(!doc) return CounterModel.incrementSeq({ name, step });
    return doc.nextVal;
  };
  ...
  CounterModel.initializeSeq = async (Model, propertyName, initialVal, step) => {
    ...
    if (!nextVal) {
      ...
      nextVal = (highestRecord && highestRecord[propertyName]) ? (highestRecord[propertyName] + step) : 1;
    }
